"""information-hub — source fetchers (collect layer).

Fetches candidate stories from free public sources for one collection:

  - ``rss``         RSS/Atom feeds (feedparser)
  - ``arxiv``       arXiv API (Atom)
  - ``hackernews``  Hacker News Firebase API (top stories)
  - ``github``      GitHub repository search, star-sorted, no auth

Each fetcher returns raw entry dicts; :func:`fetch_collection` normalises
them into :class:`~src.models.candidate.Candidate` objects and caps/dedups
by URL.
...
def fetch_rss(url: str, timeout: int = 20) -> list[dict[str, Any]]:
    """Parse an RSS/Atom feed into raw entries (title/url/summary/published)."""
    try:
...
def fetch_arxiv(query: str, max_results: int = 20, timeout: int = 20) -> list[dict[str, Any]]:
    """Query the arXiv API (Atom) for papers matching ``query``."""
...
def fetch_hackernews(max_results: int = 20, timeout: int = 20) -> list[dict[str, Any]]:
    """Top Hacker News stories via the Firebase API (no auth)."""
    try:
        resp = requests.get(
...
# type -> fetcher mapping (raw entry -> Candidate)
...
def fetch_collection(collection_name: str, sources: list[dict[str, Any]],
    ...
    Args:
        collection_name: collection name (namespacing the candidates).
        sources: list of ``{"type", ...}`` source configs from config.yml.
        max_candidates: hard cap on the number of candidates returned.

    Returns:
        Unique ``Candidate`` list (deduped by URL within this fetch), capped.
    """
...
def _source_name(src_type: str, src: dict[str, Any]) -> str:
    """Human-friendly source display name (arXiv / github:<topic> / host)."""
    if src_type == "arxiv":
        return "arXiv"
    if src_type == "github":
...
def _parse_iso(text: str) -> dt.datetime | None:
    """Best-effort ISO date parse (returns UTC-aware datetime or None)."""
    text = text.strip()
...
def _feed_date(entry: Any) -> str | None:
    """Extract the ISO publish date from a feedparser entry, if present."""
    try:
