"""information-hub — duplicate control (collect layer).
...
3. (the LLM selector receives duplicate flags and excludes duplicates)

Role: phase collect — consumed by ``collect.prompts`` (selector prompt),
``collect.mock`` (mock selector) and ``storage.registry`` (checksums).
"""
