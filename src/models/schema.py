"""information-hub — deep-dive content schema (models layer).

Every stored record must conform to this enforced JSON Schema
(``jsonschema``) plus word-count and required-field checks.  Also includes
the provenance / grounding / review blocks stamped by the pipeline.

Role: shared contract — consumed by main (validation) and seed.
"""
